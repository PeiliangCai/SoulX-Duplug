...
CONFIG="${1:-configs/stage2_paper_all.yaml}"
if [[ $# -gt 0 ]]; then
  shift
fi
...
echo "[train-stage2] config=$CONFIG"
echo "[train-stage2] config_device=${CONFIG_DEVICE:-auto} visible_gpu_count=$GPU_COUNT"

if [[ "$GPU_COUNT" -gt 1 && "$CONFIG_DEVICE" != "cpu" ]]; then
  exec torchrun --standalone --nproc_per_node "$GPU_COUNT" \
    -m soulx_duplug.train.stage2_streaming_asr \
    --config "$CONFIG" \
    "$@"
fi

exec python -m soulx_duplug.train.stage2_streaming_asr \
  --config "$CONFIG" \
  "$@"
